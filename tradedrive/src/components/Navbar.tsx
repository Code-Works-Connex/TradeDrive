      ...
      <Box sx={{ flexGrow: 1 }}>
        ...
        >
          <Toolbar sx={{ justifyContent: { xs: 'space-between', md: 'flex-start' }, alignItems: 'center' }}>
                      ...
                      >
                        {item.subItems.map((subItem, idx) => (
                          <React.Fragment key={subItem.href}>
                            ...
                            >
                              <Link href={subItem.href} style={{ textDecoration: 'none', color: 'white' }}>
                                {subItem.text}
                              </Link>
                            </MenuItem>
                          </React.Fragment>
            ...
            {/* Contact Us Button */}
            <Box sx={{ display: { xs: 'none', md: 'flex' }, ml: 'auto', mr: { md: '10%' } }}>
              <Button
                component={Link}
                href="/contact"
                sx={{
              ...
              {menuItems.map(item => (
                <React.Fragment key={item.href}>
                  <ListItem disablePadding>
                    <ListItemButton
                      onClick={item.subItems ? toggleMobileSubMenu(item.href) : toggleDrawer(false)}
                      sx={{
                    ...
                    >
                      ...
                      {item.subItems && <ExpandMoreIcon sx={{ color: 'white', transform: mobileSubMenuOpen === item.href ? 'rotate(180deg)' : 'none', transition: '0.3s' }} />}
                    </ListItemButton>
                  </ListItem>
                        ...
                        <Box key={sub.href} sx={{ pl: 3 }}>
                          <ListItem disablePadding>
                            <ListItemButton onClick={toggleDrawer(false)} sx={{ py: 1 }}>
                              <Link href={sub.href} style={{ textDecoration: 'none', color: 'white', width: '100%' }}>
                                <ListItemText primary={sub.text} primaryTypographyProps={{ fontSize: '14px' }} />
                              </Link>
                            </ListItemButton>
                          </ListItem>
                          {idx < item.subItems!.length - 1 && <Divider sx={{ backgroundColor: 'rgba(255,255,255,0.1)' }} />}
                        </Box>
                      ))}
                    </>
                  )}
